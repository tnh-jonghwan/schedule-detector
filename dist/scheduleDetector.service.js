...
class ScheduleDetectorService {
    ...
    async run() {
        try {
            ...
            for (const [dbRoute] of this.pools) {
    ...
    async detectInvalidVisitTypeOfMainSchedule(dbRoute) {
        ...
        if (!pool) {
            console.error(`풀을 찾을 수 없습니다: ${dbRoute}`);
            return results;
        }
        ...
        for (const database of databases) {
            ...
            if (dbName === 'amelia' || (dbName.length === 6 && dbName.substring(0, 1) === 'c')) {
                ...
                try {
                    ...
                    const rows = await this.executeQuery(pool, sqlFields);
...
//# sourceMappingURL=scheduleDetector.service.js.map